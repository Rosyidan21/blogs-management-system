    ...
    <div class="container mx-auto p-6">
        ...
        <form action="{{ route('posts.update', $post->id) }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <input type="text" name="title" value="{{ $post->title }}" required class="w-full p-3 mt-1 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
            </div>

            <!-- Content Textarea -->
            <div>
                <label for="content" class="block text-lg font-medium text-gray-700">Content:</label>
                <textarea name="content" required class="w-full p-3 mt-1 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500">{{ $post->content }}</textarea>
            </div>
            ...
            <div class="flex items-center">
                ...
                <input type="checkbox" name="published" value="1" {{ $post->published ? 'checked' : '' }} class="h-5 w-5 text-blue-500 border-gray-300 rounded focus:ring-2 focus:ring-blue-500">
